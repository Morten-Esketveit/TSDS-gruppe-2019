...
\begin{align}
    J(w) = \sum^N_{i=1} (y_i-\hat{y}_i)^2 + \alpha \cdot \sum_{j=1}^N |w_j|
    \intertext{The hyper-parameter $\alpha$ then controls the strength of the regularization. In the case of Ridge regularization, the cost function becomes:}
     J(w) = \sum^N_{i=1} (y_i-\hat{y}_i)^2 + \alpha \cdot \sum_{j=1}^N w_j^2
\end{align}
...
Prior to fitting the model, we scale the features using the StandardScaler in Python. This is done in order to avoid artificially large/small weights which could make convergence difficult using regularization.  \\
...
\begin{table}[htbp]
  ...
  \caption{Results, Prediction Model}
    ...
    \multicolumn{1}{l}{Model evaluation} & $R^2$  & \multicolumn{1}{r}{0.411} & \multicolumn{1}{r}{0.432} & \multicolumn{1}{r}{0.414} & \multicolumn{1}{r}{0.434} \\
          &       &  &  \multicolumn{1}{r}{(0.029)}     &       & \multicolumn{1}{r}{(0.029)} \\
    \multicolumn{1}{l}{Hyper parameters} & $\alpha$ & \multicolumn{2}{c}{6.684} & \multicolumn{2}{c}{6.211} \\
